...
import {
  getValuerProfileFields,
  updateContactProfileFields,
  reqUserDataKey,
  redisPrefix,
  contactFields,
  updateValuerProfileFields,
...
export class ValuerProfile {
  ...
  /**
   *   Steps to update a Valuer profile
   * - Update contact
   * - update Address
   * - Update profile
   */
  public static async update(req: Request, res: Response) {
    ...
    try {
      ...
      const addressObject = await profileRepo.findOne({
        where: { entityId, profileType: ValuerProfileType.ADDRESS },
      });
  ...
  public static async updatePassword(req: Request, res: Response) {
    try {
      ...
      const userPasswordData = await userProfileRepo.findOne({
        where: [{ entityId, profileType: ValuerProfileType.PASSWORD }],
      });
      if (!userPasswordData) {
        return res.json({
          responseCode: "000032",
